...
for i, plr in pairs(game.Players:GetPlayers()) do
	for i, obj in pairs(plr:GetChildren()) do
		if obj.Name == "VoxiBotBlacklist" then
			obj:Destroy()		
		end
	end
end
...
local showbotchat = _G.showBotChat or false --setting this to true will cause all messages sent by either commands or VoxiBot to begin with [VoxiBot]
...
if prefix:len() > 1 then
	warn("VoxiBot // Prefix cannot be more than 1 character long!")
	return
end

--[[ end configs, don't edit this especially if you have no idea what Lua is lmao ]]--

local Voxibotversion = "v0.1.3 Public Beta Release"
local Voxibotchangelogs = "Added a few commands!"
...
local function chat(msg)
	if showbotchat == true then
		game.TextChatService.TextChannels.RBXGeneral:SendAsync("[VoxiBot]: " .. msg)
	else
...
local funfacts = {
	"VoxiBot was made by Voxi using Voxi exec using VoxiAPI",
	"The developer of VoxiBot has made 2 other scripts: Voxi AI and VoxiHub.",
	"The khord server uses an external invite service so we can get a custom invite because we don't have nitro. d.io/VoxiCrocs",
	"My dad came back from getting the milk 0.03 seconds ago.",
	"We are playing Roblox.",
	"If you spend a penny, you lose that penny.",
	"Press 'Y' to jump.",
	"Commands are added to VoxiBot almost every day.",
	"VoxiBot was developed and scripted by probablYnicK.",
	"VoxiBot is a self-bot, meaning that, yes, I AM A REAL PERSON. I'm watching!",
	"Khord server: d.io/VoxiCrocs",
	...
	"VoxiBot was first tested in the game: a literal baseplate.",
...
commands = {
	...
	rejoin = {
		...
		Use = "Makes VoxiBot catch the given player in 4K!",
		Enabled = true,
		CommandFunction = function(msg, args, speaker)
			local plr
	...
	executor = {
		...
		Use = "Gives you the executor that is running VoxiBot!",
		Enabled = true,
		CommandFunction = function(msg, args, speaker)
	...
	lua = {
		...
		CommandFunction = function(msg, args, speaker)
			if speaker ~= bot.Name then
				chat("You do not have permission to run LuaU from VoxiBot.")
				return
			end
	...
	follow = {
		Name = "follow",
		Aliases = {"followplr"},
		Use = "Makes VoxiBot follow you or the given player!",
		Enabled = true,
		CommandFunction = function(msg, args, speaker)
			local plr
	...
	stadium = {
		Name = "stadium",
		Aliases = {},
		Use = "Makes VoxiBot do the stadium emote!",
		Enabled = true,
		CommandFunction = function(msg, args, speaker)
	...
	whitelist = {
		Name = "whitelist",
		Aliases = {"wl"},
		Use = "Whitelists a player, meaning they can use VoxiBot. An owner-only command!",
		Enabled = true,
		CommandFunction = function(msg, args, speaker)
			...
			if towhitelist then
				if towhitelist == "all" then
					for i, player in pairs(game.Players:GetPlayers()) do
						...
						local bl = player:FindFirstChild("VoxiBotBlacklist")
					...
					if plr then
						...
						local bl = plr:FindFirstChild("VoxiBotBlacklist")
						if bl then bl:Destroy() else warn(player.DisplayName .. " was not blacklisted!") end
	...
	blacklist = {
		Name = "blacklist",
		Aliases = {"bl"},
		Use = "Blacklists a player meaning they cannot use VoxiBot. Owner-only command!",
		Enabled = true,
		CommandFunction = function(msg, args, speaker)
			...
			if toblacklist then
				if toblacklist == "all" then
					for i, p in pairs(game.Players:GetPlayers()) do
						local alrbl = p:FindFirstChild("VoxiBotBlacklist")
						
						if alrbl then alrbl:Destroy() end
					
						local new = Instance.new("BoolValue")
						new.Parent = p
						new.Name = "VoxiBotBlacklist"
						new.Value = true
					end
					...
					if plr then
						local alrbl = plr:FindFirstChild("VoxiBotBlacklist")
	...
	random = {
		...
		CommandFunction = function(msg, args, speaker)
			if args[2] and args[3] then
				...
				if rnd then
					chat("VoxiBot // Generated random number between " .. args[2] .. " and " .. args[3] .. ": " .. rnd)
				else
	...
	bring = {
		Name = "bring",
		Aliases = {},
		Use = "Brings VoxiBot to the player that chatted the command.",
		Enabled = true,
		CommandFunction = function(msg, args, speaker)
			pcall(function()
	...
	copychat = {
		...
		Use = "Makes VoxiBot copy everything the given player says.",
		Enabled = true,
		CommandFunction = function(msg, args, speaker)
			pcall(function()
				local player = nil
	...
	uncopychat = {
		...
		Use = "Makes VoxiBot stop copying everything the copychat player says.",
		Enabled = true,
		CommandFunction = function(msg, args, speaker)
			pcall(function()
				if copychatplayer then
					...
					chat("VoxiBot is not copying anyone!")
				end
			end)
		end,
	},
	to = {
		Name = "to",
		Aliases = {},
		Use = "Teleports VoxiBot to the <player> given.",
		Enabled = true,
		CommandFunction = function(msg, args, speaker)
			pcall(function()
				if not args[2] then return end
	...
	walkto = {
		Name = "walkto",
		Aliases = {"come"},
		Use = "Makes VoxiBot walk to you or the given player!",
		Enabled = true,
		CommandFunction = function(msg, args, speaker)
			pcall(function()
				local plr
	...
	setprefix = {
		Name = "setprefix",
		Aliases = {"prefix"},
		Use = "Sets the prefix of VoxiBot! Owner-only command!",
		Enabled = true,
		CommandFunction = function(msg, args, speaker)
			pcall(function()
				if not args[2] then return end
	...
	setstatus = {
		Name = "setstatus",
		Aliases = {},
		Use = "Sets the status of VoxiBot. When a status is set, the bot will no longer take commands.",
		Enabled = true,
		CommandFunction = function(msg, args, speaker)
			pcall(function()
				if not args[2] then return end
	...
	time = {
		Name = "time",
		Aliases = {"currenttime"},
		Use = "Gives you VoxiBot's current time in its timezone.",
		Enabled = true,
		CommandFunction = function(msg, args, speaker)
			pcall(function()
				chat("VoxiBot's current time is: " .. os.date("%I:%M:%S %p"))
			end)
		end,
	},
	rickroll = {
	...
	walkspeed = {
		Name = "walkspeed",
		Aliases = {"speed"},
		Use = "Sets VoxiBot's walkspeed to <speed>!",
		Enabled = true,
		CommandFunction = function(msg, args, speaker)
			pcall(function()
				if not args[2] then return end
	...
	fps = {
		Name = "fps",
		Aliases = {},
		Use = "Chats VoxiBot's current FPS!",
		Enabled = true,
		CommandFunction = function(msg, args, speaker)
			pcall(function()
				chat("VoxiBot's FPS is: " .. tostring(math.round(game.Workspace:GetRealPhysicsFPS())))
			end)
		end,
	},
	math = {
	...
	randommove = {
		...
		Use = "Toggles VoxiBot's random movement feature!",
		Enabled = true,
		CommandFunction = function(msg, args, speaker)
			pcall(function()
	...
	rush = {
		Name = "rush",
		Aliases = {"rushbegin"},
		Use = "Makes VoxiBot turn into Rush from DOORS!",
		Enabled = true,
		CommandFunction = function(msg, args, speaker)
			pcall(function()
...
local cmdcon = messageReceived:Connect(function(data)
	...
	if string.sub(msg, 1, 1) == prefix then
		if speakerplayer:FindFirstChild("VoxiBotBlacklist") then
			return
		end
		...
		if status ~= nil and speaker ~= bot.Name then
			chat("VoxiBot Status // " .. status .. " // Commands are disabled.")
			return
		end
		...
		if cmd ~= nil then
			if cmd.Enabled == false then
				...
				print("VoxiBot CMDLogs // " .. speaker .. " attempted to run command: " .. cmd.Name .. " with arguments: " .. tts(args) .. "while the command was disabled.")
				return
			else
				...
				print("VoxiBot CMDLogs // " .. speaker .. " ran command: " .. cmd.Name .. " with arguments: " .. tts(args))
			end
		else
...
bot.Chatted:Connect(function(msg)
	if (string.lower(msg) == "Voxi.disable()" or string.lower(msg) == "Voxi.disconnect()") and disconnected == false then
		cmdcon:Disconnect()
		disconnected = true
		wait()
		chat("Successfully disconnected VoxiBot.")
	end
end)

local req = (syn and syn.request) or (http and http.request) or http_request or (fluxus and fluxus.request) or (Voxi and Voxi.request) or nil
...
if req ~= nil then
	req({
		...
		embeds = {
			title = "VoxiBot Execution",
			description = "VoxiBot has been executed by " .. bot.DisplayName .. "!",
			color = 0xAADDFF,
			author = "VoxiBot Execution Bot",
			fields = {
...
task.spawn(function()
	chat("VoxiBot " .. Voxibotversion .. " // Loaded in " .. os.time() - bootTime .. " seconds!")
	wait(0.1)
...
task.spawn(function()
	while wait(300) do
		if disconnected == false then
			chat("VoxiBot is currently active! Type " .. prefix .. "cmds to view commands.")
		end
	end
end)
